...
waybar -c "${USER_CONFIG:-"$HOME/.config/sway/tools/templates/waybar/config.jsonc"}" -s "${USER_STYLE:-"$HOME/.config/sway/tools/templates/waybar/style.css"}" > $(mktemp -t XXXX.waybar.log)
